...
class HyperPayPayment {
	...
	public function generate_html(Order $order,$payment_id){
		...
		return "<form action='".route('payment-verify',['payment'=>'hyperpay'])."' class='paymentWidgets' data-brands='".$form_brands."'></form>
			...
			wpwlOptions.createCheckout = function() {
			    return $.post(".route('payment-verify',['payment'=>'hyperpay']).")
