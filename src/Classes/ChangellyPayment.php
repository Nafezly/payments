...
class ChangellyPayment extends BaseController implements PaymentInterface
{
    ...
    {
        $this->changelly_api_key = config('nafezly-payments.CHANGELLY_API_KEY');
        $this->changelly_private_key = config('nafezly-payments.CHANGELLY_PRIVATE_KEY');
    ...
    public function pay($amount = null, $user_id = null, $user_first_name = null, $user_last_name = null, $user_email = null, $user_phone = null, $source = null)
    {
        ...
        $this->checkRequiredFields($required_fields, 'Changelly');
        ...
        $body = [
                ...
                "pending_deadline_at" => "2023-03-22T15:23:56.876Z",
            
        ];
        dd(str_replace('\/','/',json_encode($body)));
        
        $payload= "POST:/api/payment/v1/payments:". str_replace('\/','/',json_encode($body)) .":".$timestamp;
        $key= base64_encode(base64_encode(hash_hmac('sha256', hash('sha256', $payload) , $this->changelly_private_key)) . ":".$timestamp);
       ...
       ->post('https://api.pay.changelly.com/api/payment/v1/payments',$body);
        dd($response->status());
