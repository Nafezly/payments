...
{
    public $mamopayment_key;
    public $mamopayment_secret;
    public $mamopayment_api_base_url;
    public $mamopayment_checkout_base_url;
    public $verify_route_name;


    public function __construct()
    {
        $this->mamopayment_key = config('nafezly-payments.MAMOPAYMENT_KEY');
        $this->mamopayment_secret = config('nafezly-payments.MAMOPAYMENT_SECRET');
        $this->mamopayment_api_base_url = config('nafezly-payments.MAMOPAYMENT_API_BASE_URL');
        $this->mamopayment_checkout_base_url = config('nafezly-payments.MAMOPAYMENT_CHECKOUT_BASE_URL');
    ...
    public function pay($amount = null, $user_id = null, $user_first_name = null, $user_last_name = null, $user_email = null, $user_phone = null, $source = null): array
    {
        ...
        try{
            $payment_methods = \Http::withHeaders([
                'x-api-key' => $this->mamopayment_key,
                'x-api-secret' => $this->mamopayment_secret,
                'Content-Type' => 'application/json',
            ])->get($this->mamopayment_api_base_url.'/v1/payment_methods')->json();
            if(isset($payment_methods['payment_methods'])){
                if(null !== collect($payment_methods['payment_methods'])->where('type','CARD')->where('currency','USD')->first() ){
                    ...
                    $response = \Http::withHeaders([
                        ...
                        'x-api-secret' => $this->mamopayment_secret,
                        'Content-Type' => 'application/json',
                    ])->post($this->mamopayment_api_base_url.'/v1/checkout_preferences', [
                        'amount' => $this->amount,
                        ...
                        'type' => 'PAYMENT',
                        'custom_urls' => [
                            'status_changes_webhook' => route($this->verify_route_name,['payment'=>'mamopayment','payment_id'=>$unique_id,'step'=>'change']),
                            'success_payment_redirect' => route($this->verify_route_name,['payment'=>'mamopayment','payment_id'=>$unique_id]),
                            'error_payment_redirect' => route($this->verify_route_name,['payment'=>'mamopayment','payment_id'=>$unique_id]),
                        ],
                        'payer' => [
    ...
    public function verify(Request $request)
    {
        ...
        if(isset($request['entity_id'])){
            $response = Http::withHeaders([
                ...
                'x-api-secret' => $this->mamopayment_secret,
                'Content-Type' => 'application/json',
            ])->get($this->mamopayment_api_base_url."/v1/payment_orders/".$request['entity_id']);
